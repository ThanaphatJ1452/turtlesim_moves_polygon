...
def pidControl(p, i, d, target):
    global status, currentTime, previousTime, error, sumError, rateError, lastError
    ...
    if turnCCW == False:
        ...
        error = target - angleDiff

    sumError = sumError + (error * elapsedTime)
    rateError = (error - lastError)/elapsedTime

    try:
        vel_pid = (p * error) + (i * sumError) + (d * rateError)
    except:
        vel_pid = (p * error) + (i * sumError)
...
def moveSquare(polygon):
    ...
    pose_subscriber = rospy.Subscriber('/turtle1/pose', Pose, poseReceived) #subscribe turtlesim data
        ...
        if turnCCW == False:
            if is_init_pose == False:
                ...
                if dist >= squareDist:
                    ...
                    turtleVel.linear.x = pidControl(4, 0, 0, squareDist)
            ...
            if angleDiff >= angleRadius:
                ...
                turtleVel.angular.z = pidControl(4, 0, 0, angleRadius)
